...
export async function POST(req: Request) {
    ...
    const body = (await req.json().catch(() => null)) as SendDocumentBody | null;
    if (!body?.phone || !body?.document || !body?.extension) {
        return NextResponse.json({ message: "Dados inválidos" }, { status: 400 });
    }

    let res = await fetch(`${apiBase}/atendimentos/send-document`, {
        method: "POST",
        headers: {
    ...
    if (res.status === 401) {
        ...
        access = newAccess;
        res = await fetch(`${apiBase}/atendimentos/send-document`, {
            method: "POST",
            headers: {
                "Content-Type": "application/json",
                Authorization: `Bearer ${access}`,
    ...
    if (!res.ok) {
        return NextResponse.json({ message: data?.message ?? "Falha ao enviar documento" }, { status: res.status });
    }
