...
export async function GET(_: Request, { params }: Params) {
    ...
    let res = await fetch(`${apiBase}/atendimentos/conversations/${id}/messages`, {
    ...
    if (res.status === 401) {
        ...
        access = newAccess;
        res = await fetch(`${apiBase}/atendimentos/conversations/${id}/messages`, {
            headers: { Authorization: `Bearer ${access}` },
            cache: "no-store",
        });
    }
    ...
    if (!res.ok) return NextResponse.json({ message: data?.message ?? "Falha ao listar mensagens" }, { status: res.status });
