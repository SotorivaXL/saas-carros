...
export async function POST(req: Request, { params }: Params) {
    ...
    let res = await fetch(`${apiBase}/atendimentos/conversations/${id}/labels`, {
        method: "POST",
    ...
    if (res.status === 401) {
        ...
        access = newAccess;
        res = await fetch(`${apiBase}/atendimentos/conversations/${id}/labels`, {
            method: "POST",
            headers: { Authorization: `Bearer ${access}`, "Content-Type": "application/json" },
            body: JSON.stringify(payload),
        });
    ...
    if (!res.ok) return NextResponse.json({ message: data?.message ?? "Falha ao salvar etiquetas" }, { status: res.status });
