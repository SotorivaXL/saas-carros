...
export async function POST(req: Request, { params }: Params) {
    ...
    let res = await fetch(`${apiBase}/atendimentos/conversations/${id}/transfer`, {
        method: "POST",
    ...
    if (!res.ok) return NextResponse.json({ message: data?.message ?? "Falha ao transferir atendimento" }, { status: res.status });
