...
export async function POST(_: Request, { params }: Params) {
    ...
    let res = await fetch(`${apiBase}/atendimentos/conversations/${id}/start`, {
        method: "POST",
    ...
    if (res.status === 401) {
        ...
        access = newAccess;
        res = await fetch(`${apiBase}/atendimentos/conversations/${id}/start`, {
            method: "POST",
            headers: { Authorization: `Bearer ${access}` },
        });
    }
    ...
    if (!res.ok) return NextResponse.json({ message: data?.message ?? "Falha ao iniciar atendimento" }, { status: res.status });
