...
export async function POST(req: Request) {
    ...
    let res = await fetch(`${apiBase}/atendimentos/conversations/manual`, {
        method: "POST",
    ...
    if (!res.ok) return NextResponse.json({ message: data?.message ?? "Falha ao criar atendimento manual" }, { status: res.status });
