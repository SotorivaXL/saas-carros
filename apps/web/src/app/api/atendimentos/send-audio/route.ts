...
export async function POST(req: Request) {
    ...
    const body = (await req.json().catch(() => null)) as SendAudioBody | null;
    if (!body?.phone || !body?.audio) {
        return NextResponse.json({ message: "Dados inválidos" }, { status: 400 });
    }

    let res = await fetch(`${apiBase}/atendimentos/send-audio`, {
        method: "POST",
        headers: {
    ...
    if (res.status === 401) {
        ...
        access = newAccess;
        res = await fetch(`${apiBase}/atendimentos/send-audio`, {
            method: "POST",
            headers: {
                "Content-Type": "application/json",
                Authorization: `Bearer ${access}`,
    ...
    if (!res.ok) {
        return NextResponse.json({ message: data?.message ?? "Falha ao enviar áudio" }, { status: res.status });
    }
