...
export async function GET(req: Request) {
    ...
    let res = await fetch(`${apiBase}/reports/atendimentos/results${query ? `?${query}` : ""}`, {
    ...
    if (res.status === 401) {
        ...
        access = newAccess;
        res = await fetch(`${apiBase}/reports/atendimentos/results${query ? `?${query}` : ""}`, {
            headers: { Authorization: `Bearer ${access}` },
            cache: "no-store",
        });
    }
    ...
    if (!res.ok) return NextResponse.json({ message: data?.message ?? "Falha ao carregar relatório de resultados" }, { status: res.status });
