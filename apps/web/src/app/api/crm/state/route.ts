...
export async function GET() {
    ...
    let res = await fetch(`${apiBase}/crm/state`, {
    ...
    if (res.status === 401) {
        ...
        access = newAccess;
        res = await fetch(`${apiBase}/crm/state`, {
            headers: { Authorization: `Bearer ${access}` },
            cache: "no-store",
        });
    }
    ...
    if (!res.ok) return NextResponse.json({ message: data?.message ?? "Falha ao carregar CRM" }, { status: res.status });
...
export async function PUT(req: Request) {
    ...
    const data = await res.json().catch(() => null);
    if (!res.ok) return NextResponse.json({ message: data?.message ?? "Falha ao salvar CRM" }, { status: res.status });
    return NextResponse.json(data);
}
